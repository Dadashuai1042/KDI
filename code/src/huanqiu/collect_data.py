...
class NewspaperSpider:
    #初始化方法，设置与MongoDB的连接，并定义一个术语字典 term_dict，将英文武器类别映射到中文
    def __init__(self):
        self.term_dict = {
            'aircraft': "飞行器",
            'warship': "舰船舰艇",
    ...
    '''modify data'''
    #从MongoDB集合中读取数据，修改数据结构，并将其插入到另一个集合中
    ...
    '''采集主函数'''
    #定义了要爬取的武器大类
    #然后为每个大类获取子类别的URL，并为每个子类别获取所有页面的URL。然后，它调用 get_info 方法来获取每个页面的信息
    ...
    '''根据最大值，获取所有信息'''
    #使用 html_parser 方法获取页面内容，并使用 extract_data 方法提取数据，然后将其插入到MongoDB的知识库集合中
    ...
    '''modify data'''
    #从解析后的内容中提取有用信息，如名称、产国、图片、简介和属性，并将这些信息作为字典返回
