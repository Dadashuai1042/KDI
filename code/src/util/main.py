...
class KnowledgeGraph():
    def __init__(self):
        ...
        self.nodeLabels_ = ['气动布局', '生产单位', '研发单位', '动力装置', '发动机', '产国', '飞行速度', '底盘类型',
                            '发动机数量', '制造商']
    ...
    def _createRelationUtil(self, data, headLabel, headName, tailLabel, tailName, relation):
        try:
            self._createRelation(headLabel=headLabel, headName=data[headLabel], tailLabel=tailLabel,
                                 tailName=data[tailLabel], relation=relation)
        except:
    ...
    def readMilitaryData(self):
        with open(self.militaryPath_, 'r', encoding='utf-8') as file:
            ...
            for line in tqdm.tqdm(lines, desc="Processing Military Data"):
                ...
                self._createRelationUtil(data, headLabel="制造商", headName="制造商", tailLabel="产国", tailName="产国",
                                         relation="属于")
                self._createRelationUtil(data, headLabel="生产单位", headName="生产单位", tailLabel="产国",
                                         tailName="产国", relation="属于")
    ...
    def exportData(self):
        # 将 Neo4j 图数据库中的节点和关系导出到 CSV 文件中
    ...
    def _createRadarRela(self, label, relation):
        # 为雷达节点与某种类型（如技术体制、天线扫描方式等）的节点之间创建随机关系
        matcher = NodeMatcher(self.graph_)
        radars = [node for node in matcher.match('雷达')]
