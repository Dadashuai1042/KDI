...
#下载雷达相关的数据，并将其保存到 SQLite 数据库中
def askUrl(url):
    #发送 HTTP GET 请求到指定的 URL，并返回页面的 HTML 内容
...
def parserdata2(url):
    #从雷达类型的详细页面中解析数据，并将结果存储在列表 datalist 中
    html=askUrl(url)
    soup=BeautifulSoup(html,"html.parser")
...
def parserdata3(url):
    #从雷达详细参数页面中解析数据，并将结果存储在列表 valuelist 中
    datalist=[]
    try:
...
def get_data(url):
    #使用 askUrl 获取 URL 的 HTML 内容，并使用 parserdata 解析数据
    datalist=[]
    html=askUrl(url)
    #print(html)
...
def initdb(path):
    #初始化 SQLite 数据库，创建一个名为 movie 的表
    sql='''
    create table movie 
    (
